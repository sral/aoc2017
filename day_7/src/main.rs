...
// upon a tower of programs that have gotten themselves into a bit of trouble.
...
fn main() {
    ...
    while let Ok(bytes) = io::stdin().read_line(&mut buf) {
        ...
        {
            let mut i = buf.trim().split(" -> ");
            let root = i.next().unwrap().split(" ").nth(0).unwrap();
            match i.next() {
                Some(v) => {
                    seen_roots.insert(root.to_owned());
                    for child in v.split(", ") {
                        seen_children.insert(child.to_owned());
                    }
                }
                None => (),
            }
    ...
    println!("root: {:?}", seen_roots.difference(&seen_children).nth(0).unwrap());
}
